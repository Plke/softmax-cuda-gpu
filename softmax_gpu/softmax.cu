...
int main(int argc, char *argv[])
{
	...
	*/
	...
	int W_columns = 784;
	float* W;
	cudaMallocManaged((void **)&W , W_rows*W_columns*sizeof(float));
	weight_initialization( W , W_rows, W_columns);

	/*
	* Training data, activation and gradient buffers *
	*/
	 float *activations_train,*data_transposed_train,*delta,*W_grad,*activations_test,*data_transposed_test;
	 cudaMallocManaged((void **)&activations_train,W_rows * number_of_samples_train * sizeof(float));
	 cudaMallocManaged((void **)&data_transposed_train,rows * columns * number_of_samples_train * sizeof(float));
	 cudaMallocManaged((void **)&delta,W_rows*number_of_samples_train*sizeof(float));
	 cudaMallocManaged((void **)&W_grad,W_rows*W_columns*sizeof(float));

	/*
	* Test data and activation buffers *
	*/
	 cudaMallocManaged((void **)&activations_test,W_rows * number_of_samples_test * sizeof(float));
	 cudaMallocManaged((void **)&data_transposed_test,rows * columns * number_of_samples_test * sizeof(float));
	...
	*/
	...
	for(int epoch=0; epoch<epoch_num; epoch++)
    {
        ...
        */
        ...
        matrix_multiply( W, data_transposed_train , activations_train , 10,784,784,number_of_samples_train);
        ...
        */
        ...
        softmax(activations_train,10,number_of_samples_train);


        // Todo 3
        // loss on training set

        loss_train=cross_entropy_loss(activations_train,labels_train,10,number_of_samples_train);

        // Todo 4
        // accuracy on training set

        acc_train=accuracy(activations_train,labels_train,10,number_of_samples_train);

        // Todo 5
        // [Test] Forward on test set
        // activations_test = W * data_transposed_test

        matrix_multiply( W, data_transposed_test , activations_test , 10,784,784,number_of_samples_test);

        // Todo 6
        // [Test] Softmax on activations_test

        softmax(activations_test,10,number_of_samples_test);

        // Todo 7
        // [Test] loss on test set
        loss_test=cross_entropy_loss(activations_test,labels_test,10,number_of_samples_test);

        // Todo 8
        // [Test] accuracy on test set

        acc_test= accuracy (activations_test,labels_test,10,number_of_samples_test);
        ...
        delta_compute(delta,activations_train,labels_train,W_rows,number_of_samples_train);

        // Todo 10
        // W_grad [10,784] = delta [10,n] * images_train_set [n,784]

        matrix_multiply( delta, data_train , W_grad , 10,number_of_samples_train,number_of_samples_train,W_columns);

        // Todo 11
        // Update, W = W - alpha * W_grad;

        matrix_add(W_grad,W,10 ,784,-learning_rate);
        printf("[GPU][Epoch %d]: Train loss:%f, Train accuracy:%f;\n", epoch, loss_train, acc_train);
        printf("                 Test loss:%f, Test accuracy:%f ; time cost: %lf s\n\n",  loss_test, acc_test, cpuSecond()-time_begin);
    }
    cudaFree(delta);
    cudaFree(data_train);
    cudaFree(W_grad);
    cudaFree(activations_train);
    cudaFree(labels_train);
    cudaFree(activations_test);
    cudaFree(labels_test);
    cudaFree(data_transposed_test);
    cudaFree(W);
    cudaFree(data_transposed_train);
